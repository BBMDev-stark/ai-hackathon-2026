...
const EXAM_START = new Date('2026-05-29T09:00:00')
...
function TimerUnit({ value, label }: { value: number; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      ...
      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-400/70">{label}</span>
    </div>
  )
}
...
export default function ExamAnnouncement() {
  ...
  const pct = isFinished
    ? 100
    : isLive
    ? Math.min(100, ((now.getTime() - EXAM_START.getTime()) / TOTAL_DURATION) * 100)
    : 0
      ...
      <div className="relative max-w-7xl mx-auto px-5 sm:px-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-14">
          ...
          <p className="text-slate-500 max-w-sm text-[15px] leading-relaxed lg:text-right">
            ...
            {isUpcoming && 'Cuộc thi bắt đầu lúc 09:00, ngày 29/05/2026.'}
            {isLive && <strong className="text-slate-700">Cuộc thi đang diễn ra!</strong>}
        ...
        <div className="grid lg:grid-cols-5 gap-6 mb-8">
          ...
          <div className="lg:col-span-3 rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            ...
            <div className="p-8 sm:p-10">
              ...
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-50 border border-blue-100 mb-6">
                ...
                <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-blue-600">Đề thi chính thức</span>
              </div>

              {/* Chủ đề */}
              <div className="mb-8">
                <p className="text-[11px] font-bold uppercase tracking-[0.15em] text-slate-400 mb-2">Chủ đề</p>
              ...
              {/* Thách thức */}
              <div className="mb-8">
                <p className="text-[20px] font-bold uppercase tracking-[0.15em] text-slate-400 mb-3">Thách thức</p>
                <p className="text-[18px] text-slate-600 leading-[1.8]">
                  Dựa trên <strong className="text-slate-800">3 trụ cột</strong> bên dưới, đội của bạn hãy chọn ra{' '}
                  ...
                  <strong className="text-slate-800">giải pháp đột phá</strong> để giải quyết nó.
                </p>
              </div>
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                {pillars.map((p) => (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-xl ${p.lightBg} ${p.iconColor} flex items-center justify-center mb-3`}>
                      {p.icon}
                    </div>
                    <p className="text-[13px] font-bold text-slate-400 mb-1">Trụ cột {p.num}</p>
                    <h4 className="text-[18px] font-bold text-slate-800 leading-snug mb-2">{p.title}</h4>
                    <p className="text-[16px] text-slate-500 leading-relaxed">{p.desc}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="lg:col-span-2 flex flex-col gap-4">
              ...
              <div className="relative p-7 sm:p-8 flex flex-col h-full">
                <div className="mb-6">
                  <p className="text-[9px] font-black text-blue-400 uppercase tracking-[0.25em] mb-1">
                    {isUpcoming ? 'Đếm ngược khai mạc' : isLive ? 'Thời gian còn lại' : 'Cuộc thi kết thúc'}
                  </p>
                  <p className="text-[13px] text-slate-400">
                    {isUpcoming && '09:00 → 14:30 · 29/05/2026'}
                ...
                <div className="flex flex-col items-center gap-6 flex-1 justify-center">
                  <div className="relative w-[130px] h-[130px]">
                    <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
                      <circle cx="60" cy="60" r={radius} fill="none" strokeWidth="6" stroke="rgba(255,255,255,0.06)" />
                      <circle
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-[10px] text-slate-500 mb-0.5">
                        {isUpcoming ? 'Bắt đầu sau' : isLive ? 'Còn lại' : 'Hết giờ'}
                      </span>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-2xl font-light text-slate-600 mb-5">:</span>
                  ...
                  {isLive && (
                    <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-green-500/15 border border-green-400/30">
                      ...
                      <span className="text-[11px] font-bold text-green-400 uppercase tracking-widest">Đang diễn ra</span>
                    </div>
                  )}
                  {isUpcoming && (
                    <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-blue-500/15 border border-blue-400/30">
                      <span className="w-2 h-2 rounded-full bg-blue-400 animate-pulse" />
                      <span className="text-[11px] font-bold text-blue-400 uppercase tracking-widest">Chưa bắt đầu</span>
                    </div>
                  )}
                  {isFinished && (
                    <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-amber-500/20 border border-amber-400/30">
                      ...
                      <span className="text-[11px] font-bold text-amber-400 uppercase tracking-widest">Hết giờ!</span>
                    </div>
                  )}
                </div>
                ...
                <div className="mt-6 pt-5 border-t border-white/[0.08] grid grid-cols-2 gap-3">
                  <div>
                    <p className="text-[9px] font-bold uppercase tracking-widest text-slate-500 mb-1">Bắt đầu</p>
                    <p className="text-[13px] font-bold text-white">09:00</p>
                    <p className="text-[10px] text-slate-500">29/05/2026</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[9px] font-bold uppercase tracking-widest text-slate-500 mb-1">Kết thúc</p>
            ...
            <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
              <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400 mb-4">Lưu ý quan trọng</p>
              <ul className="space-y-3">
                {[
                  ...
                  { icon: '', text: 'Thời gian làm bài: 5 giờ (09:00 – 14:30)' },
                ].map((item) => (
                  <li key={item.text} className="flex items-start gap-3 text-[13px] text-slate-600">
        ...
        <div className="rounded-2xl border border-blue-100 bg-gradient-to-r from-blue-600 to-indigo-600 overflow-hidden shadow-lg shadow-blue-200/50">
          <div className="px-8 py-7 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-white font-bold text-[15px] leading-snug">Sẵn sàng kiến tạo tương lai?</p>
                <p className="text-blue-100/80 text-[13px]">Đề thi yêu cầu tư duy sáng tạo và khả năng ứng dụng AI thực tiễn.</p>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <span className="text-[11px] font-bold text-blue-200 uppercase tracking-widest">Hackathon · AI Discovery 2026</span>
